...
def parse_resume_text(text):
    ...
    mobile_number_pattern = r"'mobile_number':\s*'([^']*)'"
    no_of_pages_pattern = r"'no_of_pages':\s*(\d+|None)"
    skills_pattern = r"'skills':\s*\[([^\]]*)\]"

    # Extract data using regex
    name = re.search(name_pattern, generated_text)
    email = re.search(email_pattern, generated_text)
    mobile_number = re.search(mobile_number_pattern, generated_text)
    no_of_pages = re.search(no_of_pages_pattern, generated_text)
    skills = re.search(skills_pattern, generated_text)

    # Parse and structure the extracted data
    data = {
        'name': name.group(1) if name else None,
        'email': email.group(1) if email else None,
        'mobile_number': mobile_number.group(1) if mobile_number else None,
        'no_of_pages': int(no_of_pages.group(1)) if no_of_pages and no_of_pages.group(1).isdigit() else None,
        'skills': [skill.strip() for skill in skills.group(1).split(',')] if skills and skills.group(1).strip() else []
    }
